...
public partial class RhythmGameGameplay : VisualElement
{
    ...
    public RhythmGameGameplay()
    {
        ...
        for (int i = 0; i < s_images.Length; i++)
        {
            ...
            _lanes[i]
                .Add(
                    new Image()
                    {
                        sprite = UiUtils.LoadSprite("alaska_flag", Scene.StoryMode),
                        style =
                        {
                            height = UiUtils.GetLengthPercentage(10),
                            width = UiUtils.GetLengthPercentage(100),
        ...
        foreach (string image in s_images)
        {
            Image imageEl = new()
            {
                sprite = UiUtils.LoadSprite(image, Scene.StoryMode),
                style =
                {
                    height = UiUtils.GetLengthPercentage(100),
                    width = UiUtils.GetLengthPercentage(100 / s_images.Length),
    ...
    public void Play(string song)
    {
        ...
        {
            UiManager.Instance.StoryModeScreen.RhythmGameScene.OnRhythmGameFinish();
            return;
        }
    ...
    {
        if (UiManager.Instance.StoryModeScreen.CurrentScene != UiManager.Instance.StoryModeScreen.RhythmGameScene)
        {
            return;
        }    
